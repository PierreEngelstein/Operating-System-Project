...
int createIDTEntry64(IDT_Entry_64 *entry, unsigned long irq_address)
{
        ...
        entry->type_attr = 0x8e; //Interrupt gate
        entry->zero_1 = 0;
        entry->zero_2 = 0;
        return 0;
}

int createEmptyIDTEntry64(IDT_Entry_64 *entry)
{
...
int idt_init()
{
        IDT = (IDT_Entry_64 *)0x100000; //For the moment mapped statically
        /* Clear out the entire IDT */
        for(int i = 0; i < 256 * sizeof(IDT_Entry_64); i++)
                createEmptyIDTEntry64(&IDT[i]);
        ...
        idtr idt_desc;
        idt_desc.limit = sizeof(IDT_Entry_64) * 256 - 1;
        ...
        for(int i = 0; i < 16; i++)
                createIDTEntry64(&(IDT[i + 32]), (unsigned long)irqs[i]);
